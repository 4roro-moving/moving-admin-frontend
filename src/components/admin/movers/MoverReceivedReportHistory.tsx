...
export default function MoverReceivedReportHistory({
  ...
  return (
    <CustomerHistoryCard title="피신고 이력" totalCount={history.totalCount}>
      {history.items.length ? (
        <div className="overflow-x-auto">
          <div className="min-w-[560px]">
            <div className="grid grid-cols-[1.25fr_1fr_8rem_6rem] gap-3 border-b border-border bg-background-muted px-5 py-3 text-xs font-semibold text-muted">
              <span>신고 사유</span>
              <span>처리 상태</span>
              <span>접수일</span>
              <span>관리</span>
            ...
            <div className="divide-y divide-border">
              {history.items.map((item) => (
                <div
                  key={item.id}
                  className="grid grid-cols-[1.25fr_1fr_8rem_6rem] items-center gap-3 px-5 py-4"
                >
