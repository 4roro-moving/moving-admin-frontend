...
interface AdminFaqDeleteModalProps {
  faq: AdminFaq;
  isPending: boolean;
  onClose: () => void;
  onConfirm: () => void;
  ...
  return (
    ...
    >
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="admin-faq-delete-modal-title"
        className="w-full max-w-md rounded-2xl border border-border bg-surface p-6 shadow-lg"
        onClick={(event) => event.stopPropagation()}
      >
        <h2 id="admin-faq-delete-modal-title" className="text-xl font-semibold text-foreground">
          FAQ 삭제
        </h2>
        <p className="mt-3 text-sm leading-6 text-muted">
          <span className="font-medium text-foreground">{faq.question}</span>
          {" "}항목을 삭제합니다. 삭제 후에는 목록에서 다시 불러올 수 없습니다.
        </p>
