      ...
      {history.items.length ? (
        <div className="overflow-x-auto">
          <div className="min-w-[700px]">
            <div className="grid grid-cols-[8rem_minmax(20rem,1fr)_10rem_8rem] gap-4 border-b border-border bg-background-muted px-5 py-3 text-xs font-semibold text-muted">
              <span>조치</span>
              <span>사유</span>
              <span>처리일</span>
              <span>처리자</span>
            ...
            <div className="divide-y divide-border">
              {history.items.map((item) => (
                <div
                  key={item.id}
                  className="grid grid-cols-[8rem_minmax(20rem,1fr)_10rem_8rem] items-center gap-4 px-5 py-4"
                >
                  <span
                    className={
                      item.action === "SUSPEND"
